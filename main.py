...
uploaded_file = st.sidebar.file_uploader('Drop the Chats here : Time format must be 24-Hour')
if uploaded_file is not None:
    ...
    if st.sidebar.button('Show Analysis'):
        ...
        ax.set(xlabel='Activity', ylabel='Day')
        st.pyplot(fig)

    if selected_user == 'All Users':
        st.title('Users Activity')
        x, new_df = helper.user_activity(df)
        fig , ax = plt.subplots()
